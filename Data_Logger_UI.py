...
close_plot_button = ttk.Button(root, text="Close Plot",
                               command=lambda: plotting.close_plot(plot_frame, close_plot_button))
...
def read_serial_data():
    """
    Reads data from the serial port, parses it, and adds it to the queue.
    Replace 'COM_PORT' and 'BAUD_RATE' with the actual values for your hardware.
    """
    # Configure the serial port
    ser = serial.Serial(port='/dev/ttyS0', baudrate=9600, timeout=2)  # Update with your actual port and baud rate
        ...
        try:
            ...
            if raw_data:
                ...
                if len(serial_data) >= 12:  # Update this if you expect more or fewer columns
                    ...
                    power = round(current * voltage, 2)  # Calculate power

                    # Insert calculated power and timestamp
                    serial_data.insert(0, timestamp)
                    serial_data.insert(4, power)
                    ...
                    # Add data to the queue for GUI updates
                    data_queue.put(serial_data)

        except Exception as e:
            print(f"Error reading serial data: {e}")

        time.sleep(1)  # Adjust if necessary, depending on your data rate

    # Close the serial port when the thread exits
    ser.close()
